...
public class HoroscopeController {
    ...
    public ResponseEntity<ResHoroscopeDto> findHoroscopeByConstellation(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(horoscopeService.findHoroscope(constellationName));
    }

    @ApiOperation(value = "운세 아이디로 운세 보기 - date(TIME ZONE X, 단순 날짜)")
    @ApiResponses({
            @ApiResponse(code = 200, message = "운세 아이디로 운세 보기"),
